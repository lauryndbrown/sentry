...
class AcceptOrganizationInvite<P extends Props, S extends State> extends AsyncView<P, S> {
  ...
  handleAcceptInvite = async () => {
    ...
    try {
      ...
      browserHistory.push(`/${this.state.inviteDetails.orgSlug}/`);
    } catch {
      ...
      <React.Fragment>
        ...
        {inviteDetails.needsSso && (
          <p data-test-id="suggests-sso">
            {tct(
              `Note that [orgSlug] has enabled Single-Singn-On (SSO) using
  ...
  get warning2fa() {
    ...
    return (
      <React.Fragment>
        ...
        <Actions>
          <Button priority="primary" to="/account/settings/2fa/">
